...
def _search(returndate, orgnid, destid, depart, searchtype, cabin, request):
    ...
    for i in range(0,len(departlist)):
        ...
        if is_scrape_virgin_atlantic == 1:
            ...
            subprocess.Popen(["python", settings.BASE_DIR+"/pexproject/scrapers/virgin_atlantic.py",orgncode,destcode, str(depart).strip(), str(returndate).strip(), str(searchkeyid),str(returnkey)])
...
def _check_data(departkey, returnkey, cabin, allkey, checkdata=False):
    ...
    if allkey:  # multi city
        ...
        if returnkey:   # for round trip
            returnfare = "p2." + cabin
            departfare = "p1." + cabin           
            checkdata = checkdata & len(list(Flightdata.objects.raw("select p1.rowid from pexproject_flightdata p1 inner join pexproject_flightdata p2 on p1.datasource = p2.datasource and p2.searchkeyid ="+str(returnkey)+" and "+returnfare+" > 0 where p1.searchkeyid="+str(departkey)+" and "+departfare+" > 0 limit 1")))
